		...
		if (((bool(::nme::AssetData_obj::type->exists(id)) && bool((::nme::AssetData_obj::type->get(id) == ::openfl::AssetType_obj::IMAGE))))){
			...
			if (((id.indexOf(HX_CSTRING(":"),null()) > (int)-1))){
				...
				if ((::nme::AssetData_obj::library->exists(libraryName))){
					HX_STACK_LINE(129)
					if (((::nme::AssetData_obj::library->get(libraryName) == ::openfl::LibraryType_obj::SWF))){
						HX_STACK_LINE(135)
						if ((!(::openfl::Assets_obj::cachedSWFLibraries->exists(libraryName)))){
							HX_STACK_LINE(135)
							::openfl::Assets_obj::cachedSWFLibraries->set(libraryName,::format::SWF_obj::__new(::openfl::Assets_obj::getBytes(((HX_CSTRING("libraries/") + libraryName) + HX_CSTRING(".swf")))));
						}
						HX_STACK_LINE(151)
						return ::openfl::Assets_obj::cachedSWFLibraries->get(libraryName)->__Field(HX_CSTRING("getBitmapData"),true)(symbolName);
					}
				}
				else{
					HX_STACK_LINE(173)
...
::flash::display::MovieClip Assets_obj::getMovieClip( ::String id){
	...
	if ((::nme::AssetData_obj::library->exists(libraryName))){
		...
		if (((::nme::AssetData_obj::library->get(libraryName) == ::openfl::LibraryType_obj::SWF))){
			...
			HX_STACK_LINE(324)
			return ::openfl::Assets_obj::cachedSWFLibraries->get(libraryName)->__Field(HX_CSTRING("createMovieClip"),true)(symbolName);
		}
	}
	else{
		HX_STACK_LINE(346)
