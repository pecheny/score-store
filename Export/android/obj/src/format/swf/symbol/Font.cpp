		...
		while(((_g < numGlyphs))){
			...
			while((true)){
				...
				if ((!(edge))){
					...
					Void run(::flash::display::Graphics g,::flash::geom::Matrix m){
					...
					if ((lineTo)){
						...
						HX_BEGIN_LOCAL_FUNC_S4(hx::LocalFunc,_Function_6_1,Array< Float >,cx,Array< Float >,cy,Array< Float >,px,Array< Float >,py)
						Void run(::flash::display::Graphics g,::flash::geom::Matrix m){
						...
						HX_STACK_LINE(225)
						commands->__Field(HX_CSTRING("push"),true)( Dynamic(new _Function_6_1(cx,cy,px,py)));
					}
				}
			}
...
Void Font_obj::renderGlyph( ::flash::display::Graphics graphics,int index,::flash::geom::Matrix matrix){
{
...
Float Font_obj::renderCharacter( ::flash::display::Graphics graphics,int code,::flash::geom::Matrix matrix){
