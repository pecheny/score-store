...
HX_STACK_PUSH("Button::new","format/swf/symbol/Button.hx",23);
{
	...
	while(((stream->readByte() != (int)0))){
		...
		HX_STACK_LINE(63)
		::flash::geom::Matrix placeMatrix = stream->readMatrix();		HX_STACK_VAR(placeMatrix,"placeMatrix");
		HX_STACK_LINE(65)
		if (((version == (int)2))){
			HX_STACK_LINE(67)
			::flash::geom::ColorTransform colorTransform = stream->readColorTransform(true);		HX_STACK_VAR(colorTransform,"colorTransform");
			HX_STACK_LINE(69)
			if ((hasFilterList)){
				HX_STACK_LINE(69)
...
Void Button_obj::apply( ::flash::display::SimpleButton simpleButton){
{
