						...
						::format::swf::symbol::Font f = _switch_1->__Param(0);
{
		...
		struct _Function_2_1{
			inline static Dynamic Block( ::format::swf::symbol::Font &font,Array< int > &advances,bool &hasX,int &offsetX,bool &hasFont,bool &hasY,int &offsetY1,Array< int > &glyphs,Float &height,int &color,Float &alpha,bool &hasColor){
		...
		HX_STACK_LINE(103)
		this->records->__Field(HX_CSTRING("push"),true)(_Function_2_1::Block(font,advances,hasX,offsetX,hasFont,hasY,offsetY1,glyphs,height,color,alpha,hasColor));
		HX_STACK_LINE(118)
		stream->alignBits();
	}
}
...
Void StaticText_obj::render( ::flash::display::Graphics graphics){
{
		HX_STACK_PUSH("StaticText::render","format/swf/symbol/StaticText.hx",125);
		HX_STACK_THIS(this);
		HX_STACK_ARG(graphics,"graphics");
		HX_STACK_LINE(127)
		::flash::geom::Matrix matrix = null();		HX_STACK_VAR(matrix,"matrix");
		HX_STACK_LINE(128)
		::flash::geom::Matrix cacheMatrix = null();		HX_STACK_VAR(cacheMatrix,"cacheMatrix");
		HX_STACK_LINE(130)
		{
			HX_STACK_LINE(130)
			int _g = (int)0;		HX_STACK_VAR(_g,"_g");
...
Dynamic StaticText_obj::__SetField(const ::String &inName,const Dynamic &inValue,bool inCallProp)
{
	...
	case 7:
		...
		break;
	case 10:
		if (HX_FIELD_EQ(inName,"textMatrix") ) { textMatrix=inValue.Cast< ::flash::geom::Matrix >(); return inValue; }
	}
